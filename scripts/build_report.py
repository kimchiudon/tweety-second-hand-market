...
def build(args) -> None:
    ...
    story.extend([
        ...
        p("Tweety<br/>Second-hand Market", st["title"]),
        ...
        Spacer(1, 30 * mm),
        table([
            ["작성자", args.student], ["분반", f"{args.class_number}반"], ["작성일", "2026-07-23"],
            ["Public GitHub", args.repo_url], ["검증 결과", "자동 테스트 30/30 성공 · 실제 브라우저 검수 완료"],
    ...
    story.extend([
        ...
        table([
            ...
            ["검색", "상품명 한 글자 일치 검색", "바인딩 쿼리, 입력 길이 제한"],
    ...
    story.extend([
        Spacer(1, 5 * mm),
        p("최종 결과", st["h2"]),
        bullet("자동 보안·기능 테스트 30개 전부 성공", st["body"]),
    ...
    story.extend([
        ...
        table([
            ...
            ["FR-06", "검색", "상품명에서 검색어의 한 글자 이상이 일치하는 상품을 검색한다."],
            ["FR-07", "관리", "관리자가 사용자, 상품, 신고를 확인·관리한다."],
        ], [18 * mm, 30 * mm, 112 * mm], st),
        p("비기능 요구사항", st["h2"]),
        bullet("Python 3.11 이상과 Pillow 의존성을 명시해 동일 환경에서 재현 가능하게 실행", st["body"]),
    ...
    weaknesses = [
        ...
        ["문자열 SQL", "SQL 삽입", "영숫자 32자 제한·전 값 바인딩", "OR/UNION/DROP/LIKE 공격"],
        ["요청 권한값 신뢰", "관리자 권한 탈취", "DB 세션 해시·만료·ID·현재 역할 공통 검사", "mb_level/role 위조·역할 강등"],
    ...
    story.extend([
        ...
        table([
            ...
            ["4", "XSS·SQLi", "script escape, OR/UNION/DROP/LIKE 공격 후 DB 보존"],
            ...
            ["28", "채팅 신고 권한", "실제 대화 참여자만 신고 가능, 관리자 상품 맥락 확인"],
            ["29", "회원탈퇴", "재인증·익명화·전 세션 폐기·상품 숨김·복구 차단"],
            ["30", "카테고리 필터", "허용 목록·검색 결합·차단/정지 판매자 비노출"],
        ], [10 * mm, 60 * mm, 90 * mm], st),
        p("실행 결과: 30 tests / 30 passed / 0 failed / 0 errors", st["callout"]),
        PageBreak(),
    ])
    ...
    story.extend([
        ...
        table([
            ["구성", "적용 내용"],
            ["Dockerfile", "Python 3.13.11·Gunicorn, 비루트 사용자, 상태 확인"],
            ["Compose", "읽기 전용 루트, capability 제거, no-new-privileges"],
            ["데이터", "DB와 사진을 tweety-data 영구 볼륨에 저장"],
            ["비밀값", "관리자 비밀번호를 이미지가 아닌 .env 환경변수로 주입"],
            ["검증", "Compose 구성 검사와 별도 test 이미지 단계에서 30개 테스트"],
        ], [34 * mm, 126 * mm], st),
        p("유지보수 계획", st["h2"]),
        bullet("인증·세션·거래·관리자 변경 시 30개 전체 회귀 테스트 실행", st["body"]),
    ...
    story.extend([
        ...
        table([
            ...
            ["테스트", "공격·경계·권한 테스트 생성", "30개 자동 테스트 전부 실행"],
        ...
        p("결론", st["h2"]),
        p("Tweety는 수업 자료의 가입, 상품, 소통, 차단, 송금, 검색, 관리자 요구를 모두 충족한다. 비밀번호·세션·입력·권한·상태·오류·로그를 한 개발 주기 안에서 함께 다뤘으며, 자동 테스트와 수동 브라우저 검증으로 주요 약점의 제거 여부를 확인했다. Dockerfile과 Compose로 비루트·읽기 전용 실행환경과 영구 데이터 볼륨을 제공하고, 남은 운영 위험은 README와 보안 정책에 명시했다.", st["body"]),
        Spacer(1, 8 * mm),
